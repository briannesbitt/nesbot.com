...
function writePosts($posts, $file)
{
   ...
   $s .= sprintf('$postsOrder = array(); $posts = array();%s', PHP_EOL, PHP_EOL);

   $cnt = count($posts) - 1;
   foreach ($posts as $post) {
      $s .= sprintf('$postsOrder["%s"] = %s;%s', $post->slug, $cnt, PHP_EOL);
...
class Posts implements IPosts
{
   ...
   public function __construct(array $postsOrder, array $posts)
   {
...
$posts = new Posts($postsOrder, $posts);
